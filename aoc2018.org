...
** COMMENT Commentary
* todo Day 06: Chronal Coordinates :pmap:min_key:dissoc:
:PROPERTIES:
:EXPORT_FILE_NAME: day06
:EXPORT_HUGO_WEIGHT: 106
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :summary "Optimizing with pmap; groupping with min-key, dissociating hash-maps and more!"
:END:
** Puzzle summary
...
#+begin_src txt :tangle inputs/day06-ex
1, 1
1, 6
8, 3
3, 4
...
** Solution
#+name: day06_ns
#+begin_src clojure :noweb-ref code06 :exports none :results silent
...
#+name: day06_test_ns
#+begin_src clojure :noweb-ref test06 :exports none :results silent
...
*** Manhattan distance
The [[https://en.wikipedia.org/wiki/Taxicab_geometry#Formal_definition][formula]] for...
...
#+name: day06_manhattan_test
#+begin_src clojure :noweb-ref test06
...
#+name: day06_manhattan
#+begin_src clojure :noweb-ref code06 :exports code :results silent
...
#+begin_src clojure :noweb-ref test06 :results silent :exports code
...
#+name: day06_group_by_doc
#+begin_src clojure :ns aoc18.puzzle06 :exports results
(doc group-by)
#+end_src

#+RESULTS: day06_group_by_doc
: -------------------------
...
So, given point ~4, 0~ from the [[day06_example_grid][example]] we will get a hash-map where distances are ~keys~ (results of ~mahnattan~ func fed by the point and coordinate), and ~values~ are coordinates compared to the point:

#+name: day06_group_by_example
#+begin_src clojure :ns aoc18.puzzle06 :exports both :results code :noweb-ref example
(group-by #(manhattan [4 0]  %) (input->xy "day06-ex"))
#+end_src

#+RESULTS: day06_group_by_example
#+begin_src clojure
...
#+name: day06_min_key_docs
#+begin_src clojure :ns aoc18.puzzle06 :exports results
(doc min-key)
#+end_src

#+RESULTS: day06_min_key_docs
: -------------------------
...
#+name: day06_min_key_example
#+begin_src clojure :exports code :results silent
  (min-key first [9 1] [2 7])             ;;=> [2 7]
  (min-key #(first (keys %)) {9 1} {2 7}) ;;=> {2 7}
#+end_src

Since output of our [[day06_group_by_example][example]] is a ~hash-map~ we need to ~apply~ ~min-key~ to the whole using ~key~ a filtering func (it wasn't obvious for me at first):

#+name: day06_min_key_over_example1
#+begin_src clojure :exports code :results silent
  (apply min-key key {4 ['(1 1)], 9 ['(1 6)], 7 ['(8 3)],
                      5 ['(3 4)], 6 ['(5 5)], 13 ['(8 9)]}) ;;=> [4 [(1 1)]]
#+end_src

The same we'd do with a vector of vectors, etc.:
#+name: day06_min_key_over_example2
#+begin_src clojure :exports code :results silent
  (apply min-key first [[9 2] [1 7]])     ;;=> [1 7]
#+end_src

Alright we have to add only one condition: we are interested only in points which have /one/ closest coordinate. Our func may then look like this:

#+name: day06_closest
#+begin_src clojure :noweb-ref code06 :exports code
...
*** Making a grid
To get all relevant points of the grid we need to find minimal and maximal ~x, y~ coordinates, like this (assuming that `coords` are from the [[day06_example_grid][example]]):

#+name: day06_getting_mins_and_maxs
#+begin_src clojure :exports code :results silent
  (reduce #(map min %1 %2) [##Inf ##Inf] coords) ;;=> (1 1)
  (reduce #(map max %1 %2) [0 0] coords)         ;;=> (8 9)
#+end_src

Then we'd use a ~list comprehension~ to create the grid:

#+name: day06_make_grid
#+begin_src clojure :noweb-ref code06 :exports code
...
*** Addendum: the border
This is what was missing in my first (not working) solution: to exclude areas considered infinite we have to find closest coordinates to the border of the grid. The border will consist only of points in range from minx to maxs and miny/maxy (upper and lower sides of the grid) or minx/max and range from miny to maxy (left and right) sides of the grid.

#+name: day06_make_border_sketch
#+begin_src clojure :exports code
  (defn make-border [[minx miny] [maxx maxy]]
    (concat (for [x (range minx (inc maxx))] [x miny])
            (for [x (range minx (inc maxx))] [x maxy])
            (for [y (range miny (inc maxy))] [minx y])
            (for [y (range miny (inc maxy))] [maxx y])))
#+end_src

To make it more concise let's play with ~apply~ and ~list~:

#+name: day06_make_border
#+begin_src clojure :noweb-ref code06 :exports code
(defn make-border [[minx miny] [maxx maxy]]
...
Let's test it agains example data:
#+name: day06_make_border_test
#+begin_src clojure :noweb-ref test06 :exports code
  (deftest make-border-test
    (testing "should match exemplary data"
      (let [coords (input->xy "day06-ex")
            mins (reduce #(map min %1 %2) [##Inf ##Inf] coords)
            maxs (reduce #(map max %1 %2) [0 0] coords)
            border '([1 1] [1 9] [2 1] [2 9] [3 1] [3 9] [4 1] [4 9] [5 1]
                     [5 9] [6 1] [6 9] [7 1] [7 9] [8 1] [8 9] [1 1] [8 1]
                     [1 2] [8 2] [1 3] [8 3] [1 4] [8 4] [1 5] [8 5] [1 6]
                     [8 6] [1 7] [8 7] [1 8] [8 8] [1 9] [8 9])]
        (is (= (sort border) (sort (make-border mins maxs)))))))

#+end_src

To get coordinates closest to the border, we may use ~keep~ iin the

*** Encountering computational problems

*** Putting things together
#+name: day06_tangle_code
#+begin_src clojure :noweb yes :tangle src/aoc18/puzzle06.clj :results silent :exports none
  <<code06>>
#+end_src

#+name: day06_tangle_tests
#+begin_src clojure :noweb yes :tangle test/aoc18/puzzle06_test.clj :exports results  :exports none
  <<test06>>
#+end_src

#+name: day06_tests
#+begin_src clojure :results output :exports results
  (require '[clojure.test :refer [run-tests]] 'aoc18.puzzle06-test)
  (run-tests 'aoc18.puzzle06-test)
#+end_src
...
#+RESULTS: day06_tangle_tests

** COMMENT Notes
For example, in the plane, the taxicab distance between {\displaystyle (p_{1},p_{2})}(p_{1},p_{2}) and {\displaystyle (q_{1},q_{2})}(q_{1},q_{2}) is {\displaystyle |p_{1}-q_{1}|+|p_{2}-q_{2}|.}|p_{1}-q_{1}|+|p_{2}-q_{2}|.

* todo Day 07: The Sum of Its Parts
:PROPERTIES:
:EXPORT_FILE_NAME: day07
